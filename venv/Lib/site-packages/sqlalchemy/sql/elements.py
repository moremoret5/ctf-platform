    ...
    def columns(
        ...
        **types: _TypeEngineArgument[Any],
        ...
        name or origin of the column in the textual SQL doesn't matter::
            ...
            stmt = text(
                "SELECT user.id, addresses.id, user.id, "
                "user.name, addresses.email_address AS email "
                "FROM user JOIN addresses ON user.id=addresses.user_id "
                "WHERE user.id = 1"
            ).columns(
                User.id,
                Address.id,
...
class Case(ColumnElement[_T]):
    ...
    # user would have to cast() the case to get a specific type
