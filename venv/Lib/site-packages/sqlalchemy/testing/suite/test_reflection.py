...
class ComponentReflectionTest(ComparesTables, OneConnectionTablesTest):
    ...
    def define_views(cls, metadata, schema):
        if testing.requires.materialized_views.enabled:
            ...
            materialized = set()
        for table_name in ("user", "email_addresses", "dingalings"):
            fullname = table_name
            if schema:
    ...
    def exp_fks(
        ...
        tables = {
            (schema, "user"): [
                fk(["parent_user_id"], ["user_id"], "user", name="user_id_fk")
            ],
            (schema, "dingalings"): [
                fk(["id_user"], ["user_id"], "user"),
                fk(
                    ["address_id"],
                    ["address_id"],
                    "email_addresses",
            ...
            (schema, "email_addresses"): [
                fk(["remote_user_id"], ["user_id"], "user")
            ],
            (schema, "comment_test"): [],
            (schema, "no_constraints"): [],
            (schema, "local_table"): [
    ...
    def test_get_table_names(self, connection, order_by, use_schema):
        ...
        if order_by == "foreign_key":
            answer = ["user", "email_addresses", "dingalings"]
            eq_(table_names, answer)
        else:
            answer = ["dingalings", "email_addresses", "user"]
    ...
    def _test_get_comments(self, connection, schema=None):
        ...
        eq_(
            insp.get_table_comment("user", schema=schema),
            exp[(schema, "user")],
        )
    ...
    def test_get_columns(self, connection, use_views, use_schema):
        ...
        if use_views:
            ...
            table_names = ["user", "email_addresses"]
    ...
    def test_get_indexes(self, connection, use_schema):
        ...
        insp = inspect(connection)
        indexes = insp.get_indexes("user", schema=schema)
        exp = self.exp_indexes(schema=schema)
        self._check_list(
            indexes, exp[(schema, "user")], self._required_index_keys
        )
    ...
    def test_get_view_definition_does_not_exist(self, connection):
        ...
        with expect_raises(NoSuchTableError):
            insp.get_view_definition("user")  # a table
    ...
    def test_get_table_options(self, use_schema):
        ...
        if testing.requires.reflect_table_options.enabled:
            res = insp.get_table_options("user", schema=schema)
            is_true(isinstance(res, dict))
            ...
            with expect_raises(NotImplementedError):
                insp.get_table_options("user", schema=schema)
    ...
    def get_multi_exp(self, connection):
        def provide_fixture(
            ...
            if ObjectKind.TABLE in kind:
                filter_names.extend(
                    ["comment_test", "user", "does-not-exist"]
                )
            if ObjectKind.VIEW in kind:
